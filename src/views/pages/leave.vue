...
//                    let {id, name, userName, department, immediateLeader, immediateLeaderId, leaveDateBegin, leaveDateEnd, successor, leaveReason, leaveType} = result;
      ...
      handleSubmit () {
        ...
        if(Count === 12){
          ...
          axios.post(config.devBaseUrl+'/application/addApplication', this.applicationData)
          .then((res) => {
            if(res.data.status === 1){
      ...
      queryRecord () {
        // 查询申请记录
        axios.get(config.devBaseUrl+'/application/queryRecord?id='+this.getCurrentStaffId()+'&type=1&pageSize=5')
        .then((res) => {
          if(res.data.status == 1) {
      ...
      // 查询待审批记录
      queryNotApprove () {
          axios.post(config.devBaseUrl+'/application/queryNotApprove', {id: this.getCurrentStaffId(), role: this.getCurrentRole()})
            .then((res) => {
                if(res.data.status===1){
                    let result = res.data.result.list;
                    result.forEach((item) => {
...
//                  this.$Message.success(res.data.msg)
        ...
        // 查询名下已审批记录
      queryApprove () {
          axios.post(config.devBaseUrl+'/application/queryApprove', {id: this.getCurrentStaffId(), role: this.getCurrentRole()})
            .then((res) => {
                if(res.data.status===1){
                    let result = res.data.result.list;
                    result.forEach((item) => {
        ...
        // 删除申请
        del (_id) {
            axios.post(config.devBaseUrl+'/application/delApplication', {_id})
                .then((res) => {
                    if(res.data.status===1){
                      this.$Message.success(res.data.msg);
                      this.queryRecord();
        ...
        agreeApplication (_id,id,role) {
            axios.post(config.devBaseUrl+'/application/approve', {_id, role: this.getCurrentRole(), action: 1})
                .then((res) => {
                    if(res.data.status===1){
                      this.$Message.success(res.data.msg);
        ...
        refuseApplication (_id,id,role) {
            axios.post(config.devBaseUrl+'/application/approve', {_id, role: this.getCurrentRole(), action: 0})
            .then((res) => {
                if(res.data.status===1){
                    this.$Message.success(res.data.msg);
                    this.queryNotApprove();
